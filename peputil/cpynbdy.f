...
C
     ...
     1 BODY(6,5,8),MON(6,40),MASS(10),MEQINC/0D0/
     ...
     2 JDM1,JDM2,JDM0,JVEL,IVLL,J1,J2,JDP,PINT5
     ...
     1 INTBD,KBDY(40),KKBDY(80)
      REAL*10 PREC(3,3),TEMP(6)
      REAL*4 EPSBD,PSID(40),EPSD(40),LIBRT(40,3)
      CHARACTER*8 TITLM(11)/'  MOON  ','ECKERT-B','ROWN LUN','AR THEOR',
     1  'Y 2000-1','750 WITH',' VELOCIT','Y,NUTATI','ON,LIBRA',
     2  'TION    ','12/11/69'/
...
C        INITIALIZE ITEMS NOT CARRIED BY OLD NBODY1 OUTPUT...
     ...
     1  RELFTB/10*1D0/, LEVP/12*'    '/
C
      NAMELIST/INPUT/ IBODY,JBODY,IMN,ITST,SKPHDR,JD1,JD2,SPAN,NSKIP,
     1 NPLNT,LIST,STOP,TITLM,IAU80N,J2000,TJ2000,CRDIN,CRDOUT,
...
C FILE.  IF LEVEL1 IS BLANK, THE TAPE WAS WRITTEN BY A PRE-'A' VERSION.
         IF(CRDOUT) THEN
...
C READ FURTHER HEADER INFORMATION.  SOURCE DEPENDS ON VERSION OF NBODY1
     ...
     1       (RELFTB(I),I=1,NBDY)
            IF(LEVELI.EQ.LV1A) READ(IBODY,140) (LEVP(I),I=1,NBDY2)
         ENDIF
      ELSE
         IF(TAPERR.EQ.0) THEN
...
C         GET ROTATION MATRIX TO J2000
  ...
  169    FORMAT(' PRECESSION MATRIX FROM B1950.0 TO J2000:'/
     1    (3F20.16))
      ENDIF
C
  170 WRITE(6,171) JDBD1,JDBD2,NBDY
...
C
      IF(IMN.GT.0) THEN
         ...
         JDM0  =0
         JDM2  =JDBD2-10*IDIR
         WRITE(IMN) MPLNT,MCENTR,IPAR,INTM,JDM1,JDM0,JDM2,IZERO,IZERO,
     1   IZERO,IZERO
      ENDIF
     ...
     .    (BETA(I,NDMPBD),I=1,6),IZERO,IZERO,IZERO,IZERO
      ENDIF
C
      WRITE(6,407) JDBD
