...
C     OLDSPT= T IF OUTPUT FILE IS TO HAVE ONLY 3 COORDS PER SPOT (DEF=F)
C     STOP  = T IF OUTPUT SHOULD STOP AFTER HEADERS (DEF=F)
C     EIGEN = T IF PROGRAM IS TO PRINT MATRIX EIGENVALUES INSTEAD OF
C               COPYING (DEF=F)
 
      INTEGER*4 NMPRM,NMBOD,NMPSR
      PARAMETER (NMPRM=100,NMBOD=30,NMPSR=24)
      INTEGER*2 ICREST(1000),ICT(80),IDREST(1000),
     . KSCRD(100),LCHAR(209),LDT(600),LEQN(3,400),
     . LPHS(9,200),LPL(NMBOD),LPRM(100),LPSRCN(NMPSR,10),LRBS(2,200),
     . LSCRD(6,100),LSHAPE(1000),LSHAR(209),LSKYCF(80,10),
     . LSPCRD(6,100),LZHAR(1000),NCENTR,NCPHS(200),NNMPSR,NPLNT,
     ...
     . NUMSTR,NZONE,NZONE1,NSITCO,NSITCSV,NSPTCR,NSPTCO
     ...
     . PRMTER(100),PSRCN(NMPSR,10),SCORD(6,100),SHAR(209),SKYCF(80,10),
     . SPCORD(6,100),T0SIT(100),ZHAR(500),ZNSQPP,ZNSQSN,ZSMPP,ZSMSN,
     . SUMAPS,T0SPOT(100)
     ...
     . SPTPSR(10),FLGAPS
      ...
      LOGICAL*4 CRDIN,CRDOUT,EIGEN,OLDSIT,OLDSPT,STOP
...
C
  ...
  103    FORMAT(20I4)
     ...
     .    NUMPH2,IPPR,NSITCR,NNMPSR,NSPTCR,
     .    (DT(I),JDDT(I),LDT(I),I=1,NUMDT1)
  104    FORMAT(5(16I5/), 15I5/ 4I5/ (2(4X,E17.9,I10,I5)))
...
C    .            NUMPH2,NSHP2X,(NSHAP(I),I=1,6),JDDT0,LNKLVL
     ...
     .    JDDT0,LNKLVL,NSITCR,NNMPSR,NSPTCR
      ENDIF
      NSHP2X = -1
      NUMPH2 = NUMPHR+2
      DO I=NPRMX+1,NMPRM
      ...
      NSITCO=NSITCR
      IF(OLDSIT) NSITCO=3
      IF(NNMPSR.EQ.0) NNMPSR=16
      IF(NSPTCR.LE.0) NSPTCR=3
      NSPTCO=NSPTCR
      IF(OLDSPT) NSPTCO=3
      IF(.NOT.EIGEN) THEN
         IF(CRDOUT) THEN
...
99100       FORMAT(10A8/ A8,A4,I10,25X,A4,2I4)
     ...
     .       NPARAM,ITERAT,NPAGE,NUMPLN,NUMPHR,NUMSIT,NUMSPT,
     .       NUMSTR,NUMPSR,NUMRBS,NUMEQN,NUMPHS,NUMDT,NUMDT1,
     .       NUMPH2,IPPR,NSITCO,NNMPSR,NSPTCO,
     .       (DT(I),JDDT(I),LDT(I),I=1,NUMDT1)
...
C SPOTS
      IF(NUMSPT.GT.0) THEN
         IF(CRDIN) THEN
            IF(EXTP.EQ.EXTPO) THEN
               IF(NSPTCR.LT.6) THEN
                  READ(IMATI,340) (SPOT(I),NSPLNT(I),
     .             (LSPCRD(J,I),J=1,3),(SPCORD(J,I),J=1,3), I=1,NUMSPT)
  340             FORMAT(A4,I5,3I2/ 1P,3D26.19)
               ELSE
                  READ(IMATI,1340) (SPOT(I),NSPLNT(I),T0SPOT(I),
     .             (LSPCRD(J,I),J=1,6),(SPCORD(J,I),J=1,6), I=1,NUMSPT)
 1340             FORMAT(A4,I5,1PD26.19,6I2/3D26.19/3D26.19)
               ENDIF
            ELSE
  ...
  341          FORMAT(A4,I5,3I2/ 1P,3D25.17)
     ...
     .         (SPCORD(J,I),J=1,3),(LSPCRD(J,I),J=1,3),I=1,NUMSPT)
            ELSE
               READ(IMATI) (SPOT(I), NSPLNT(I),T0SPOT(I),
     .         (SPCORD(J,I),J=1,6),(LSPCRD(J,I),J=1,6),I=1,NUMSPT)
            ENDIF
         ENDIF
         IF(.NOT.EIGEN) THEN
            IF(CRDOUT) THEN
               IF(NSPTCO.LT.6) THEN
                  WRITE(IMATS,340) (SPOT(I),NSPLNT(I),
     .             (LSPCRD(J,I),J=1,3),(SPCORD(J,I),J=1,3), I=1,NUMSPT)
               ELSE
                  WRITE(IMATS,1340) (SPOT(I),NSPLNT(I),T0SPOT(I),
     .             (LSPCRD(J,I),J=1,6),(SPCORD(J,I),J=1,6), I=1,NUMSPT)
               ENDIF
            ELSE
               IF(NSPTCO.LT.6) THEN
     ...
     .             (SPCORD(J,I),J=1,3),(LSPCRD(J,I),J=1,3),I=1,NUMSPT)
               ELSE
                  WRITE(IMATS) (SPOT(I),NSPLNT(I),T0SPOT(I),
     .             (SPCORD(J,I),J=1,6),(LSPCRD(J,I),J=1,6),I=1,NUMSPT)
               ENDIF
            ENDIF
         ENDIF
...
C PULSAR PARAMETERS
      IF(NUMPSR.GT.0) THEN
         IF(CRDIN) THEN
            DO I=1,NUMPSR
               IF(EXTP.EQ.EXTPO) THEN
                  READ(IMATI,367) SPTPSR(I),JDPSR0(I),PLSPR(I),
     .             NTYPSR(I),(PSRCN(J,I),J=1,NNMPSR)
  367             FORMAT(A4,I8,1PD26.19,I5,3X,D26.19/ (3D26.19))
               ELSE
                  READ(IMATI,371) SPTPSR(I),JDPSR0(I),PLSPR(I),
     .             NTYPSR(I),(PSRCN(J,I),J=1,NNMPSR)
  371             FORMAT(A4,I8,1PD25.17,I5,3X,D25.17/ (3D25.17))
               ENDIF
               READ(IMATI,105) (LPSRCN(J,I),J=1,NNMPSR)
            END DO
         ELSE
            READ(IMATI) (SPTPSR(I),JDPSR0(I),PLSPR(I),NTYPSR(I),
     .       (PSRCN(J,I),J=1,NNMPSR), (LPSRCN(J,I),J=1,NNMPSR),
     .       I=1,NUMPSR)
         ENDIF
         IF(.NOT.EIGEN) THEN
            IF(CRDOUT) THEN
               DO I=1,NUMPSR
                  WRITE(IMATS,367) SPTPSR(I),JDPSR0(I),PLSPR(I),
     .             NTYPSR(I),(PSRCN(J,I),J=1,NNMPSR)
                  WRITE(IMATS,105) (LPSRCN(J,I),J=1,NNMPSR)
               END DO
            ELSE
...
C PHASE CORRECTIONS
  ...
  400 IF(CRDIN) THEN
     ...
     .       (ERMES(I),I=1,3),ZNSQSN,ZSMSN,IAPRIO,IVECTK,FLGAPS
  410       FORMAT(3I10,1P,2E17.9/ 3D26.19/ 2D26.19,2I5,1XA4)
         ELSE
            READ(IMATI,411,END=900) MESMT,NTAPE,NSEQ,(ERWGT(I),I=1,2),
     .       (ERMES(I),I=1,3),ZNSQSN,ZSMSN,IAPRIO,IVECTK,FLGAPS
  411       FORMAT(3I10,1P,2E17.9/ 3D25.17/ 2D25.17,2I5,1XA4)
         ENDIF
         IF(FLGAPS.EQ.'####') THEN
            READ(IMATI,412,END=900) SUMAPS
  412       FORMAT(1P,D26.19)
         ELSE
            SUMAPS=0
         ENDIF
      ELSE
         READ(IMATI,END=900) MESMT,NTAPE,NSEQ,(ERWGT(I),I=1,2),
     .       (ERMES(I),I=1,3),ZNSQSN,ZSMSN,IAPRIO,IVECTK,SUMAPS
      ENDIF
